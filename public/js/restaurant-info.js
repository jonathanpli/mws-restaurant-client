(async() => {
   ...
   */
  const initMap = async() => {
    await fetchRestaurantFromURL(async(error, restaurant) => {
  ...
  const fetchRestaurantFromURL = async(callback) => {
    if (self.restaurant) { // restaurant already fetched!
      return await callback(null, self.restaurant);
    }
    ...
    if (!id) { // no id found in URL
      ...
      await DBHelper.fetchRestaurantById(id, async(error, restaurant) => {
        ...
        return await callback(null, restaurant);
      });
    }
  };
   ...
   */
  const getParameterByName = async(name, url) => {
    if (!url)
